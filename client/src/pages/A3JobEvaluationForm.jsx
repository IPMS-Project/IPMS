...
const renderSignaturePreview = (field) => {
    ...
    if (sig.type === 'draw') {
      return <img src={sig.value} alt="Signature" style={{ maxHeight: '40px' }} />;
    }
    if (sig.type === 'text') {
      return <span style={{ fontFamily: sig.font, fontSize: '24px' }}>{sig.value}</span>;
    }
    ...
    <div className="page-content">
     <h2 className="heading-maroon">A.3 – Job Performance Evaluation</h2>
      <Container className="p-4 rounded shadow-lg" style={{ backgroundColor: '#fff', maxWidth: '900px', width: '100%' }}>
        <Form onSubmit={handleSubmit}>
          <Table bordered responsive className="text-center custom-table">
            <thead>
              <tr><th>Item</th><th>Satisfactory</th><th>Unsatisfactory</th><th>Comments</th></tr>
            </thead>
            <tbody>
              {evaluationItems.map((item, index) => (
                <tr key={item}>
                  <td>{item}</td>
                  <td><Form.Check type="radio" name={`item-${index}`} value="Satisfactory" checked={ratings[item] === 'Satisfactory'} onChange={() => handleRatingChange(item, 'Satisfactory')} required /></td>
                  <td><Form.Check type="radio" name={`item-${index}`} value="Unsatisfactory" checked={ratings[item] === 'Unsatisfactory'} onChange={() => handleRatingChange(item, 'Unsatisfactory')} /></td>
                  <td><Form.Control as="textarea" rows={2} value={comments[item] || ''} onChange={(e) => handleCommentChange(item, e.target.value)} placeholder="Enter comments" style={{ minWidth: '250px' }} /></td>
                </tr>
              ))}
            </tbody>
          </Table>
          ...
          <Row className="mb-4">
            <Col md={6} className="mb-3 mb-md-0">
              <Form.Group>
                ...
                <div style={{ cursor: 'pointer', borderBottom: '1px solid #ccc', minHeight: '40px', padding: '6px 0' }} onClick={() => { setActiveSignatureTarget('advisor'); setShowModal(true); }}>
                  {renderSignaturePreview('advisorSignature')}
                </div>
                <Form.Check type="checkbox" className="mt-2" label="I agree that by typing/drawing my name, I am electronically signing this document." checked={formData.advisorAgreement} onChange={(e) => handleChange('advisorAgreement', e.target.checked)} required />
              </Form.Group>
            </Col>
            <Col md={6}>
              <Form.Group>
                <Form.Label>Internship Coordinator Signature</Form.Label>
                <div style={{ cursor: 'pointer', borderBottom: '1px solid #ccc', minHeight: '40px', padding: '6px 0' }} onClick={() => { setActiveSignatureTarget('coordinator'); setShowModal(true); }}>
                  {renderSignaturePreview('coordinatorSignature')}
                </div>
                <Form.Check type="checkbox" className="mt-2" label="I agree that by typing/drawing my name, I am electronically signing this document." checked={formData.coordinatorAgreement} onChange={(e) => handleChange('coordinatorAgreement', e.target.checked)} required />
              </Form.Group>
            </Col>
          </Row>
          ...
          <div className="text-center">
            <Button type="submit" className="px-5 text-white" style={{ backgroundColor: '#9d2235', borderColor: '#9d2235' }}>Submit Evaluation</Button>
          </div>
        </Form>
      </Container>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} centered dialogClassName="custom-signature-modal">
        <Modal.Body className="rounded shadow bg-white p-4" style={{ border: 'none', borderRadius: '16px' }}>
          ...
          <Tab.Container activeKey={activeTab} onSelect={setActiveTab}>
            ...
            <Tab.Content>
              <Tab.Pane eventKey="type">
                <Form.Control type="text" placeholder="Your name" value={typedSignatures[activeSignatureTarget]} onChange={(e) => setTypedSignatures(prev => ({ ...prev, [activeSignatureTarget]: e.target.value }))} className="mb-3" />
