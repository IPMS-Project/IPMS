...
async function insertFormData(formData) {
  try {
    ...
    const duplicate = await Users.findOne({ email: formData.interneeEmail });
    // if (duplicate) {
    //   return { status: "duplicate", message: "This email already exists" };
    // }
    // const duplicate = await Users.findOne({ email: formData.interneeEmail });
    ...
    if (formData.status === "submitted") {
      ...
      formData.coordinator_status="" // no approval needed ?? TBD??
